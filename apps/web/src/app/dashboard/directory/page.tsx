...
export default function DirectoryPage() {
    ...
    useEffect(() => {
        const loadLdapConfig = async () => {
            try {
                const config = await directoryApi.getConfig();
                if (config) {
                    setLdapConfig({
                        url: config.url || '',
    ...
    const tenantUrl = `${getPublicApiBaseUrl()}/scim/v2`;

    const handleGenerateToken = async () => {
        setLoading(true);
        setMessage('');
        try {
            const response = await directoryApi.generateScimToken();
    ...
    const handleLdapSave = async () => {
        ...
        setMessage('');
        try {
            await directoryApi.updateConfig(ldapConfig);
