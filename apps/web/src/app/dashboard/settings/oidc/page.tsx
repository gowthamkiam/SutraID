...
export default function OidcSettingsPage() {
    ...
    const loadConfig = async (appId: string) => {
        try {
            setLoading(true);
            const data = await oidcConfigApi.getConfig(appId);
            setConfig(data);
        } catch (err) {
        ...
        <div style={{ padding: '24px', maxWidth: '1200px', margin: '0 auto' }}>
            <header style={{ marginBottom: '32px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                ...
                <div style={{ display: 'flex', gap: '12px' }}>
                    <button onClick={() => {
                        if (selectedAppId) {
                            const discoveryUrl = applicationApi.getOidcDiscoveryUrl(selectedAppId);
            ...
            <main style={{
                ...
                {activeTab === 'scopes' && <ScopesTab scopes={config?.scopes || []} appId={selectedAppId!} onRefresh={() => loadConfig(selectedAppId!)} canUpdate={canUpdate} />}
                {activeTab === 'claims' && <ClaimsTab claims={config?.claims || []} regexRules={config?.regexRules || []} appId={selectedAppId!} onRefresh={() => loadConfig(selectedAppId!)} canUpdate={canUpdate} />}
                {activeTab === 'regex' && <RegexRulesTab rules={config?.regexRules || []} appId={selectedAppId!} onRefresh={() => loadConfig(selectedAppId!)} canUpdate={canUpdate} />}
                {activeTab === 'security' && <SecurityTab policy={config?.tokenPolicy} appId={selectedAppId!} onRefresh={() => loadConfig(selectedAppId!)} canUpdate={canUpdate} />}
                {activeTab === 'keys' && <KeysTab keys={config?.signingKeys || []} appId={selectedAppId!} onRefresh={() => loadConfig(selectedAppId!)} canUpdate={canUpdate} />}
            </main>
        </div>
    );
}

function ScopesTab({ scopes, appId, onRefresh, canUpdate }: { scopes: OidcScope[], appId: string, onRefresh: () => void, canUpdate: boolean }) {
    const [newScope, setNewScope] = useState({ name: '', description: '', isDefault: false });

    const handleAdd = async () => {
        if (!newScope.name || !canUpdate) return;
        await oidcConfigApi.createScope(appId, newScope);
...
function ClaimsTab({ claims, regexRules, appId, onRefresh, canUpdate }: { claims: OidcClaim[], regexRules: OidcRegexRule[], appId: string, onRefresh: () => void, canUpdate: boolean }) {
    ...
    const handleAddClaim = async () => {
        ...
        await oidcConfigApi.createClaim(appId, newClaim);
...
function SecurityTab({ policy, appId, onRefresh, canUpdate }: { policy: OidcTokenPolicy, appId: string, onRefresh: () => void, canUpdate: boolean }) {
    const [formData, setFormData] = useState(policy || {});

    const handleSave = async () => {
        if (!canUpdate) return;
        await oidcConfigApi.updateTokenPolicy(appId, formData);
        onRefresh();
    };
...
function KeysTab({ keys, appId, onRefresh, canUpdate }: { keys: OidcSigningKey[], appId: string, onRefresh: () => void, canUpdate: boolean }) {
    ...
    const handleUploadKey = async () => {
        ...
        await oidcConfigApi.createSigningKey(appId, newKey);
...
function RegexRulesTab({ rules, appId, onRefresh, canUpdate }: { rules: OidcRegexRule[], appId: string, onRefresh: () => void, canUpdate: boolean }) {
    ...
    const handleAdd = async () => {
        ...
        await oidcConfigApi.createRegexRule(appId, newRule);
